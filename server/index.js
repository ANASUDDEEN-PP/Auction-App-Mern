...
app.post('/api/team/register', async (req, res) => {
  ...
  .then(teamDetails => {
    console.log('New Team Added Successfully', teamDetails);
...
app.post('/api/login', async (req, res) => {
  ...
  try{
    ...
    } else if (adminLogin.AdminUserName === email && adminLogin.AdminPassword === password){
      console.log('Admin Login Successfully');
      res.status(200).json({message :'Admin Login Successfully'});
    } else {
      console.log('Check Your Username and Password');
      res.status(200).json({message :'Check Your Username and Password'});
    }
  } catch(err){
    console.log(err);
    res.status(500).json(err);
